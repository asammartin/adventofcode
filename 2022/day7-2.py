...
def findSmallestGreaterThan(tree, smallest):
    if tree["type"] == "dir":
        for child in tree["children"].values():
            smallest = findSmallestGreaterThan(child, smallest)
        if tree["size"] < smallest["size"] and tree["size"] > neededSpace:
            smallest = tree
    return smallest

smallest = findSmallestGreaterThan(fileTree, fileTree)
